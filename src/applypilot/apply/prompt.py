...
def _build_screening_section(profile: dict) -> str:
    ...
    willing_to_relocate = work_auth.get("willing_to_relocate", False)
    relocation_line = (
        f"lives in {city}, OPEN to relocating for the right role"
        if willing_to_relocate
        else f"lives in {city}, cannot relocate"
    )

    auth_status = "YES, authorized" if work_auth.get("legally_authorized_to_work") else "NOT authorized"

    return f"""== SCREENING QUESTIONS (be strategic) ==
Hard facts -> answer truthfully from the profile. No guessing. This includes:
  - Location/relocation: {relocation_line}
  - Work authorization: {auth_status} to work (legally_authorized_to_work = {work_auth.get('legally_authorized_to_work', 'see profile')})
...
def _build_hard_rules(profile: dict) -> str:
    ...
    # Build work auth rule dynamically. Always state the yes/no explicitly --
    # never let a permit-type label stand in for it, since a temporary permit
    # (e.g. OPT EAD) can otherwise read as ambiguous or "not authorized" to
    # the agent even when legally_authorized_to_work is true.
    ...
    auth_yesno = "YES" if auth_info else "NO"
    work_auth_rule = f"Work auth: currently authorized to work = {auth_yesno}."
    if permit_type:
        work_auth_rule += f" Status: {permit_type}."
    work_auth_rule += f" Sponsorship needed now or in future: {'YES' if sponsorship else 'NO'}."
...
def build_prompt(job: dict, tailored_resume: str,
    ...
    if dry_run:
        ...
        submit_instruction = """MANDATORY PRE-SUBMIT CHECKLIST -- do this as its own explicit step, not a mental note:
1. Take a fresh browser_snapshot of the fully-filled form (not the one from when you filled it -- a NEW one).
2. Go field by field through the snapshot and read back the ACTUAL value shown for: name, email, phone, location/current city, work authorization, sponsorship, resume file attached, cover letter (if applicable), and any required dropdown/radio/checkbox.
3. For each one, compare it against the APPLICANT PROFILE and TAILORED RESUME above. Any field that is blank, still shows placeholder text, shows the wrong value, or is an unselected required radio/checkbox is a FAILURE, not a minor issue.
4. If ANY field fails step 3: fix it, then repeat this whole checklist from step 1. Do not skip re-verification after a fix -- fixes on these forms frequently do not stick (framework re-renders, dropdown resets, etc).
5. Only after an entire pass of this checklist finds zero problems: click Submit/Apply.
Do NOT click Submit based on what you remember filling in earlier -- only based on what THIS checklist's fresh snapshot actually shows on screen right now. If a required field genuinely cannot be filled after real effort, that is RESULT:FAILED:stuck, not a reason to submit anyway."""
