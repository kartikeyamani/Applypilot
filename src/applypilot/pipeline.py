...
def _run_discover(workers: int = 1) -> dict:
    """Stage: Job discovery — ATS boards, Workday, JobSpy, and smart-extract scrapers.

    Order is deliberate: Ashby/Greenhouse first (public form, no personal
    login needed -- most reliable for autonomous apply), then Workday
    (also a direct employer API, no login), then JobSpy (Indeed/LinkedIn/
    etc. -- big boards, but LinkedIn applications need a logged-in session
    the automation can't always carry over), then smart-extract last.
    """
    stats: dict = {"ats_boards": None, "workday": None, "jobspy": None, "smartextract": None}

    from applypilot.config import DISCOVERY_CAP
    from applypilot.database import count_jobs

    def _cap_reached(after: str, skip: list[str]) -> bool:
        if DISCOVERY_CAP and count_jobs() >= DISCOVERY_CAP:
            console.print(
                f"  [yellow]Discovery cap of {DISCOVERY_CAP} reached after {after} — "
                f"skipping {', '.join(skip)}.[/yellow]"
            )
            for s in skip:
                stats[s] = "skipped (cap reached)"
            return True
        return False

    # Ashby / Greenhouse company boards
    console.print("  [cyan]Ashby / Greenhouse job boards...[/cyan]")
    try:
        from applypilot.discovery.ats_boards import run_ats_board_discovery
        run_ats_board_discovery(workers=workers)
        stats["ats_boards"] = "ok"
    except Exception as e:
        log.error("ATS board discovery failed: %s", e)
        console.print(f"  [red]ATS board error:[/red] {e}")
        stats["ats_boards"] = f"error: {e}"

    if _cap_reached("Ashby/Greenhouse", ["workday", "jobspy", "smartextract"]):
        return stats
    ...
    if _cap_reached("Workday", ["jobspy", "smartextract"]):
        return stats

    # JobSpy (Indeed, LinkedIn, Glassdoor, ZipRecruiter, Google Jobs)
    ...
    if _cap_reached("JobSpy", ["smartextract"]):
        return stats
