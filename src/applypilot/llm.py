...
def _detect_provider() -> tuple[str, str, str]:
    ...
    Precedence: Anthropic > Gemini > OpenAI > local. Anthropic is checked
    first because it's a deliberate opt-in (no free tier) -- if a user has
    gone and set ANTHROPIC_API_KEY, that's a clear signal to use it even if
    a GEMINI_API_KEY is still sitting in .env from initial setup.
    """
    ...
    if anthropic_key and not local_url:
        return (
            _ANTHROPIC_BASE,
            model_override or "claude-haiku-4-5-20251001",
            anthropic_key,
        )
...
class LLMClient:
    ...
    def _chat_anthropic(
        ...
        Anthropic has no OpenAI-compatible chat/completions endpoint, so this
        talks to /v1/messages directly and converts the OpenAI-style
        messages list (system/user/assistant) into Anthropic's system +
        messages shape.
        """
    ...
    def chat(
        ...
        for attempt in range(_MAX_RETRIES):
            try:
                ...
                # Covers DNS failures (getaddrinfo), connection resets, etc.
                # These are transient network blips, not provider errors --
                # previously they weren't retried at all and failed instantly.
                if attempt < _MAX_RETRIES - 1:
                    wait = min(_RATE_LIMIT_BASE_WAIT * (2 ** attempt), 60)
                    log.warning(
