...
def _setup_profile() -> dict:
    ...
    profile["work_authorization"] = {
        ...
        "willing_to_relocate": Confirm.ask("Willing to relocate for the right role?", default=False),
    }
...
def _setup_searches() -> None:
    ...
    # Cities/regions from the location answer also drive apply-stage location
    # eligibility (see apply/prompt.py::_build_location_check) -- without this,
    # that check falls back to just the profile's single city field and rejects
    # any hybrid/onsite job elsewhere, even if the user typed a broader answer here.
    accept_patterns = [c.strip() for c in location.split(",") if c.strip()]
    ...
    lines = [
        ...
        "",
        "# Cities/regions accepted for hybrid/onsite roles during auto-apply.",
        "# Genuinely remote postings are always accepted regardless of this list.",
        "location:",
        "  accept_patterns:",
    ]
...
def _setup_ai_features() -> None:
    ...
    if provider == "gemini":
        ...
        model = Prompt.ask("Model", default="gemini-3.5-flash-lite")
