    ...
    if workers > 1 and len(valid_keys) > 1:
        ...
        for key in valid_keys:
            ...
            if config.DISCOVERY_CAP:
                current_total = get_connection().execute("SELECT COUNT(*) FROM jobs").fetchone()[0]
                if current_total >= config.DISCOVERY_CAP:
                    log.info("Discovery cap of %d reached (%d jobs in DB) — stopping Workday crawl early.",
                             config.DISCOVERY_CAP, current_total)
                    break
